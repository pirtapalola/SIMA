...
Visualise the prior distributions:
 -Plot the 10,000 samples drawn from each prior distribution to a histogram.
...
prior1 = TruncatedLogNormal(loc=0, scale=5, upper_bound=5)
prior2 = TruncatedLogNormal(loc=0, scale=5, upper_bound=2)
prior3 = TruncatedLogNormal(loc=0, scale=5, upper_bound=20)
prior4 = torch.distributions.LogNormal(1.85, 0.33)

# Plot the PDF for visualization
x_values1 = np.linspace(0.01, 5, 1000)
x_values2 = np.linspace(0.01, 2, 1000)
x_values3 = np.linspace(0.01, 20, 1000)
...
prior_samples = pd.read_csv("C:/Users/pirtapalola/Documents/DPhil/Chapter2/Methods/"
                            "Methods_Ecolight/Jan2024_lognormal_priors/priors/priors_summary.csv")
samples_phy = prior_samples["Phytoplankton"].to_numpy()
samples_cdom = prior_samples["CDOM"].to_numpy()
samples_nap = prior_samples["SPM"].to_numpy()
...
axes[0, 0].hist(samples_phy, bins=1000, alpha=0.5, color='#76c893', density=True, label='Samples (n = 10,000)')
axes[0, 0].legend()
axes[0, 0].set_title('Phytoplankton')
axes[0, 0].set_xlim(left=0, right=5)  # Set x-axis limits
...
axes[0, 1].hist(samples_cdom, bins=1000, alpha=0.5, color='#76c893', density=True, label='Samples (n = 10,000)')
axes[0, 1].legend()
axes[0, 1].set_title('CDOM')
axes[0, 1].set_xlim(left=0, right=2)  # Set x-axis limits
...
axes[1, 0].hist(samples_nap, bins=1000, alpha=0.5, color='#76c893', density=True, label='Samples (n = 10,000)')
axes[1, 0].legend()
axes[1, 0].set_title('SPM')
axes[1, 0].set_xlim(left=0, right=20)  # Set x-axis limits
...
axes[1, 1].hist(samples_wind, bins=1000, alpha=0.5, color='#76c893', density=True, label='Samples (n = 10,000)')
axes[1, 1].legend()
axes[1, 1].set_title('Wind')
axes[1, 1].set_xlim(left=0, right=20)  # Set x-axis limits
axes[1, 1].set_ylim(bottom=0, top=5.0)  # Set y-axis limits
