...
import numpy as np
from modelSBI import density_estimator  # Replace with the actual module and class names
...
inference.density_estimator = density_estimator

"""

STEP 4. Apply the trained density estimator.

"""

# Create an instance of your density estimator class
density_estimator = density_estimator

# Load the state_dict
state_dict = torch.load('density_estimator.pth')
...
new_observation = obs_df['reflectance']

# Build the posterior using the loaded density estimator
posterior = inference.build_posterior(density_estimator)

# Given this observation, sample from the posterior p(θ|x), or plot it.
posterior_samples = posterior.sample((10000,), x=new_observation)

# Evaluate the log-probability of the posterior samples
log_probability = posterior.log_prob(posterior_samples, x=new_observation)
